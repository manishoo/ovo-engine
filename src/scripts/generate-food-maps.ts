...
export default async function main() {
  const recipes = await RecipeModel.find({ 'ingredients.item': { $exists: false } })
    ...
    recipe.ingredients.map(ingredient => {
      ...
      if (ingredient.name) {
        ...
        if (foundIngredient) {
          ...
          if (ingredient.unit && !foundIngredient.units!.find(p => !!ingredient.unit!.name.find(name => name.text === p.text))) {
            foundIngredient.units!.push({
              text: ingredient.unit.name[0].text,
            })
          }
        } else {
